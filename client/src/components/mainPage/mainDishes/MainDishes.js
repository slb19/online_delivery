...
const MainDishes = ({addToCart}) => {

    const foodsContext= useContext(FoodsContext)
    const { foods } = foodsContext
    return (
        <div style={{marginBottom:"10px"}} className="jumbotron" >
        <h4 style={{fontSize:"1.8em", fontStyle:"italic", textDecoration:"underline"}}>Main Dishes</h4>
        <div class="card-group">
        {foods.map(food=>{
            if(food.foodType === "MainDishes"){
...
/*
<div style={{marginBottom:"10px"}} className="jumbotron">
             <h4 style={{fontSize:"1.4em", fontStyle:"italic"}}>Main dishes</h4>
            {foods.map(food=>{
                if(food.foodType === "MainDishes"){
